...
import { fetchServicesBySalon, fetchSlotsBySalon, createBooking } from "../../redux/booking/bookingThunks";
...
import { setService, setSlot, setDate, resetBooking } from "../../redux/booking/bookingSlice";
import { MapPin, Calendar as CalIcon, Clock, CreditCard, Sparkles, ChevronRight, Loader2 } from "lucide-react";
...
export default function SalonDetails() {
  ...
  const { services, slots, selectedService, selectedSlot, selectedDate } = useSelector((s) => s.booking);
  ...
  const getMapUrl = () => {
    if (salon?.mapLink && salon.mapLink.startsWith('http')) return salon.mapLink;
      ...
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        {salon && (
          <header className="relative group overflow-hidden bg-white/[0.02] border border-white/10 rounded-[2.5rem] p-8 flex flex-col md:flex-row gap-8 items-center">
             ...
             {salon.image && (
                <img src={salon.image} alt={salon.name} className="w-64 h-44 object-cover rounded-[2rem] shadow-2xl grayscale-[0.3] group-hover:grayscale-0 transition-all duration-700" />
             )}
             <div className="flex-1 space-y-4 relative z-10 text-center md:text-left">
                ...
                <h1 className="text-5xl font-black tracking-tighter italic uppercase">{salon.name}</h1>
                <p className="text-zinc-500 flex items-center justify-center md:justify-start gap-2 text-sm">
                    <MapPin size={16} className="text-zinc-700" /> {salon.address}, {salon.city}
                </p>
                <div className="pt-2">
                  <a 
                    href={getMapUrl()} 
                  ...
                  >
                      Get Directions <ChevronRight size={14} className="group-hover/btn:translate-x-1 transition-transform" />
                  </a>
                </div>
             </div>
          </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-white/[0.02] border border-white/5 rounded-[2.5rem] p-8">
              <h2 className="text-xl font-bold mb-8 flex items-center gap-3">
                <span className="h-8 w-8 rounded-full bg-white text-black flex items-center justify-center text-xs">01</span>
                Selection_Catalog
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {services.map((s) => (
                  <button 
                    key={s._id}
                    onClick={() => { dispatch(setService(s)); dispatch(setSlot(null)); }}
                    className={`relative p-6 rounded-2xl border text-left transition-all duration-300 ${
                        selectedService?._id === s._id ? "bg-violet-600 border-violet-400 shadow-[0_0_30px_rgba(139,92,246,0.2)]" : "bg-white/5 border-white/5 hover:border-white/20"
                    }`}
                  >
                    <p className={`text-lg font-bold tracking-tight ${selectedService?._id === s._id ? "text-white" : "text-zinc-300"}`}>{s.name}</p>
                    <p className={selectedService?._id === s._id ? "text-violet-200" : "text-zinc-500"}>₹{s.price}</p>
                    {selectedService?._id === s._id && <div className="absolute top-4 right-4 h-2 w-2 bg-white rounded-full animate-ping" />}
                  </button>
                ))}
              </div>
            </section>
            ...
            {selectedService && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <section className="bg-white/[0.02] border border-white/5 rounded-[2.5rem] p-8">
                        <h2 className="text-lg font-bold mb-6 flex items-center gap-3 italic"><CalIcon size={18}/> Date_Index</h2>
                        <div className="relative group">
                          <input 
                              ...
                              onChange={(e) => { dispatch(setDate(e.target.value)); dispatch(setSlot(null)); }} 
                          />
                          {!selectedDate && (
                    ...
                    {selectedDate && (
                        <section className="bg-white/[0.02] border border-white/5 rounded-[2.5rem] p-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                            <h2 className="text-lg font-bold mb-6 flex items-center gap-3 italic"><Clock size={18}/> Time_Matrix</h2>
                            {slots.length > 0 ? (
                              <div className="grid grid-cols-3 gap-2">
                                  {slots.map((slot) => (
                                      <button key={slot._id} onClick={() => dispatch(setSlot(slot))}
                                              className={`py-2 rounded-lg border font-mono text-xs transition-all ${
                                                  selectedSlot?._id === slot._id ? "bg-white text-black border-white shadow-[0_0_15px_rgba(255,255,255,0.2)]" : "bg-white/5 border-white/5 hover:border-white/10 text-zinc-500"
                                              }`}>
                                          {slot.startTime}
                                      </button>
                                  ))}
                              ...
                              <div className="text-zinc-600 font-mono text-[10px] uppercase italic">Zero_Slots_Found</div>
                            )}
                        </section>
                    )}
                </div>
          ...
          <aside className="relative">
            <div className="sticky top-32 bg-gradient-to-br from-zinc-900 to-black border border-white/10 rounded-[2.5rem] p-8 space-y-8 overflow-hidden">
                ...
                <h2 className="text-2xl font-black italic tracking-tighter uppercase">Summary</h2>
                ...
                <div className="space-y-4 relative z-10">
                    ...
                    <div className="flex justify-between text-sm">
                        ...
                        <span className="text-zinc-200">
                          {selectedDate ? new Date(selectedDate).toLocaleDateString('en-GB', { day:'2-digit', month:'short' }) : "--"} 
                          {selectedSlot ? ` @ ${selectedSlot.startTime}` : ""}
                        </span>
                    </div>
                    <div className="h-px bg-white/10 my-4" />
                    <div className="flex justify-between items-end">
                        <span className="text-zinc-500 font-mono text-xs uppercase tracking-widest">Total_Payload</span>
                        <span className="text-3xl font-light tracking-tighter text-white">₹{selectedService?.price || 0}</span>
                    </div>
                </div>

                <button 
                  disabled={processing || !selectedService || !selectedDate || !selectedSlot}
                ...
                >
                    ...
                    <span className="relative z-10 flex items-center justify-center gap-2 group-hover:text-white transition-colors">
                        {processing ? <Loader2 className="animate-spin" /> : "Authorize Payment"}
                    </span>
                </button>
                ...
                {statusText && (
                    <div className="flex items-center justify-center gap-2 py-2 px-4 bg-white/5 rounded-full">
                        ...
                        <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">{statusText}</span>
                    </div>
                )}
            </div>
          </aside>
