...
import { 
  Calendar, Clock, CreditCard, XCircle, CheckCircle2, 
  Timer, Package, Star, ShieldCheck, Zap, Navigation, ArrowUpRight 
} from "lucide-react";
...
const statusConfig = {
  PENDING: { label: "PENDING", color: "text-amber-400", glow: "shadow-amber-500/20", bg: "bg-amber-400/10", icon: Timer },
  ACCEPTED: { label: "CONFIRMED", color: "text-emerald-400", glow: "shadow-emerald-500/20", bg: "bg-emerald-400/10", icon: CheckCircle2 },
  COMPLETED: { label: "FULFILLED", color: "text-sky-400", glow: "shadow-sky-500/20", bg: "bg-sky-400/10", icon: Package },
  CANCELLED: { label: "TERMINATED", color: "text-rose-500", glow: "shadow-rose-500/20", bg: "bg-rose-500/10", icon: XCircle }
};
        ...
        <header className="relative group p-12 rounded-[2.5rem] bg-gradient-to-br from-white/[0.03] to-transparent border border-white/5 overflow-hidden">
          ...
          >
            <h2 className="text-5xl md:text-7xl font-black tracking-tighter italic text-white uppercase leading-none italic">
              Track Your <span className="text-transparent bg-clip-text bg-gradient-to-r from-white to-zinc-600">Bookings</span>
            </h2>
    ...
    >
      ...
      <div className="relative z-10 p-8 md:p-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-12">
        ...
        <div className="flex-1 space-y-8 w-full">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-6">
              <div className="relative h-20 w-20 rounded-2xl overflow-hidden ring-1 ring-white/10 group-hover:ring-violet-500/50 transition-all">
                <img src={booking?.salon?.image || "/fallback.png"} className="h-full w-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
              </div>
              
              <div>
                <div className={`flex items-center gap-2 mb-2 ${config.color} font-mono text-[10px] tracking-tighter uppercase font-bold`}>
                  <StatusIcon size={12} />
                  <span className="bg-white/5 px-2 py-0.5 rounded-full">{config.label}</span>
                </div>
                <h3 className="text-2xl font-bold text-white tracking-tight">{booking?.salon?.name}</h3>
            ...
            <div className="lg:hidden text-right">
               <p className="text-[10px] font-mono text-zinc-600 uppercase">Amount</p>
          ...
          <div className="flex flex-wrap items-center gap-3">
             {booking?.services?.map((s, i) => (
               <span key={i} className="px-4 py-1.5 rounded-full bg-white/[0.03] border border-white/5 text-xs font-semibold text-zinc-400 group-hover:text-white group-hover:bg-violet-500/10 transition-all">
                 {s.name}
               </span>
             ))}
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-8 lg:px-12 lg:border-x lg:border-white/5">
           <DataPoint label="Schedule" value={booking?.slot?.date} icon={Calendar} />
           <DataPoint label="Timeframe" value={booking?.slot?.startTime} icon={Clock} />
           <DataPoint label="Booking ID" value={booking._id.slice(-6).toUpperCase()} isMono />
        </div>
        ...
        <div className="w-full lg:w-48 flex flex-col items-center lg:items-end justify-center">
          {isCompleted ? (
            <div className="w-full text-center lg:text-right">
              <p className="text-[10px] font-mono text-zinc-500 uppercase mb-3 tracking-widest">Feedback Loop</p>
              <div className="flex justify-center lg:justify-end gap-1.5">
                {[1, 2, 3, 4, 5].map((s) => (
                  <motion.button
                    ...
                    onClick={() => handleRating(s, booking, refresh, setSubmitting)}
                  >
                    <Star
                      size={20}
            ...
            <div className="flex flex-col items-center lg:items-end gap-4">
               <div className="hidden lg:block text-right">
                 <p className="text-[10px] font-mono text-zinc-600 uppercase">Valuation</p>
                 <p className="text-3xl font-black text-white">₹{booking?.totalAmount}</p>
               </div>
               {booking?.salon?.mapLink && (
                 <button 
                 ...
                 >
                  📍 Directions <ArrowUpRight size={14} className="group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
                 </button>
               )}
            </div>
          )}
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-1.5 opacity-40">
        {Icon && <Icon size={10} />}
        <p className="text-[9px] font-mono uppercase tracking-[0.2em]">{label}</p>
      </div>
      <p className={`text-sm font-bold tracking-tight text-zinc-200 ${isMono ? 'font-mono text-violet-400/80' : ''}`}>
        {value || "---"}
      </p>
    </div>
  );
