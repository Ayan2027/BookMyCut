...
import { LayoutGrid, Calendar, Scissors, Wallet, Plus, ArrowRight, Zap } from "lucide-react";
    ...
    <div className="relative min-h-screen bg-[#030303] text-zinc-100 p-6 lg:p-12 overflow-hidden selection:bg-violet-500/30">
      ...
      <div className="max-w-7xl mx-auto space-y-10">
          ...
          <div className="relative flex flex-col md:flex-row md:items-center justify-between bg-black/40 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-10">
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-mono uppercase tracking-[0.3em] text-violet-400/80">Hyper-Terminal v2.0</span>
              </div>
              ...
              <p className="text-zinc-500 font-medium tracking-wide">
                ...
                <span className="text-zinc-300 uppercase text-sm tracking-widest">{salon?.status}</span>
              </p>
            </div>
            
            <div className="mt-6 md:mt-0 flex flex-col items-end">
                <div className="bg-white/5 border border-white/10 px-4 py-2 rounded-full backdrop-blur-md">
                    <span className="text-sm font-mono text-violet-300">SECURE_NODE_ACTIVE</span>
                </div>
            </div>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <StatCard label="Catalog" value={services.length} unit="Services" icon={<Scissors size={24}/>} color="violet" />
          <StatCard label="Availability" value={slots.length} unit="Active Slots" icon={<Calendar size={24}/>} color="blue" />
          <StatCard label="Revenue" value="₹0" unit="Net Wallet" icon={<Wallet size={24}/>} color="emerald" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-gradient-to-br from-white/[0.05] to-transparent border border-white/10 rounded-[3rem] p-10 relative overflow-hidden group">
             <div className="relative z-10">
                ...
                <div className="flex flex-wrap gap-4">
                    <AdvancedButton label="Add Service" icon={<Plus />} onClick={() => navigate("/salon/services")} />
                    <AdvancedButton label="Sync Slots" icon={<Calendar />} onClick={() => navigate("/salon/slots")} />
                </div>
             </div>
          ...
          >
            <div className="absolute top-0 right-0 p-8">
                <ArrowRight size={40} className="-rotate-45 group-hover:rotate-0 transition-transform text-white/50" />
            </div>
            <div className="mt-20">
                <p className="text-white/60 uppercase tracking-widest text-xs font-bold mb-2">Live Monitor</p>
                <h3 className="text-3xl font-bold text-white">View<br/>Bookings</h3>
            </div>
          </button>
        </div>
...
function StatCard({ label, value, unit, icon, color }) {
  ...
  return (
    <div className={`group relative bg-white/[0.02] border border-white/10 ${colors[color]} hover:bg-white/[0.05] rounded-[2.5rem] p-8 transition-all duration-500 hover:-translate-y-2`}>
      ...
      <div>
        <h4 className="text-5xl font-light tracking-tighter mb-1 transition-transform">{value}</h4>
        <p className="text-xs text-zinc-500 font-mono tracking-widest">{unit}</p>
      </div>
    </div>
  );
}
